# Step 35 진행 상황: Sort / Limit / Distinct 실험

## 현재 상태

Step 35 완료. ORDER BY+LIMIT와 DISTINCT를 planned AST와 runtime 결과로 관찰한다. 현재 활성 CLI 저장소는 계속 `GlueSqlTaskRepository<TracingStorage<SledStorage>>`이며 기존 Project/Task/Tag 기능과 JSON/MemoryStorage 테스트 흐름을 보존한다.

## 이전 단계에서 달라진 점

| 구분 | 이전 단계 | Step 35 |
| --- | --- | --- |
| 중심 작업 | 앞 단계의 Query Lab 기반 | Sort / Limit / Distinct 실험 |
| 분석 근거 | 앞 단계에서 확보한 plan/runtime 정보 | 해당 단계의 직접 근거와 한계를 문서화 |
| 회귀 정책 | 기존 관계형 기능 유지 | 기존 명령과 테스트를 삭제하지 않음 |

## 실제 구현 위치

- `scenario_sql("sort"|"distinct")`, `Query.order_by`, `Query.limit`, `Select.distinct`
- 공통 진입점: `src/main.rs::run`, `src/query_lab/mod.rs::analyze`
- 공통 보고서: `src/query_lab/report.rs::AnalysisReport`
- 상세 해설: `docs/query-lab/09-sort-limit.md`

## 구현과 해석

Tree에는 Projection 위에 Distinct/Sort/Limit을 표시하고 consumed/returned 차이를 함께 보고한다.
...
- `renders_aggregate_sort_and_limit`, `json_report_contains_plan_and_metrics`
- 실행 시간은 assertion으로 사용하지 않는다.
- plan 종류, Storage 호출, consumed/returned/affected row와 결과값을 검증 대상으로 사용한다.

## 제약과 주의점

정렬 알고리즘, top-N 사용, LIMIT 조기 중단 위치는 public plan에 없으므로 추정하지 않는다.
