...
이 문서는 초심자가 `rust-task`를 터미널에서 직접 실행하고, 현재 Step 11의 GlueSQL SQL 실행 흐름, REPL 흐름, 테스트 보강 상태를 이해하게 돕는다.
...
이 위치에 `Cargo.toml`이 있어야 한다. `tasks.json`은 Step 7까지 사용한 JSON 저장소 파일로 남아 있지만, Step 11 기본 실행 경로에서는 사용하지 않는다.
...
현재 Step 11 기준으로 테스트는 `src/task.rs`, `src/cli.rs`, `src/error.rs`, `src/service.rs`, `src/repl.rs`, `src/repository/mod.rs`, `src/repository/gluesql_repository.rs`, `src/main.rs` 안에 있다.
...
Step 11도 GlueSQL `MemoryStorage`를 사용한다. 이 저장소는 프로그램이 끝나면 데이터가 사라지므로, 여러 CLI 명령을 나눠 실행하면 이전 데이터가 유지되지 않을 수 있다. 다만 `repl` 안에서는 같은 실행이 유지된다.
...
Step 10의 `MemoryStorage`는 실행이 끝나면 사라진다. 그래서 `cargo run -- add "Rust 공부"`를 실행한 뒤 별도 명령으로 `cargo run -- list`를 실행하면 아래처럼 비어 있을 수 있다.

```text
List:
```

같은 repository 인스턴스 안에서 `add -> list`가 이어지는지는 `src/repository/gluesql_repository.rs`의 `adds_and_lists_tasks_with_gluesql` 테스트가 검증한다.
...
주의: `done`도 별도 프로세스에서 실행하면 이전 `add` 데이터가 남아 있지 않을 수 있다. Step 11에서는 같은 인스턴스 흐름을 테스트와 REPL로 확인한다.
...
Step 11 기본 실행 경로는 `tasks.json`을 사용하지 않는다. 이 파일은 `JsonTaskRepository` 구현을 보존하기 위해 남아 있다.
...
```text
rust-task Step 11: Tested GlueSQL-backed Todo CLI
...
Note: Step 10 adds a small SQL REPL on GlueSQL InMemoryStorage.
```
...
| `list`가 예상과 다름 | Step 10의 `MemoryStorage`는 실행이 끝나면 사라짐 | 같은 인스턴스 흐름은 `cargo test` 또는 `cargo run -- repl`로 확인 |
| `Task not found: 1` | 현재 실행의 GlueSQL 저장소에 id 1이 없음 | 먼저 같은 실행 흐름 안에서 Todo를 추가해야 함 |
