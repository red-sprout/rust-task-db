...
- Step 18 문서는 `JsonTaskRepository`, `MemoryStorage`, `SledStorage`와 문서 비교 대상 storage를 구분한다. `SharedMemoryStorage`, `JsonStorage`, `MongoStorage`, `CompositeStorage`는 현재 코드에 도입하지 않았다.
