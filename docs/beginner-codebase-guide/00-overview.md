...
Rust 초심자가 `struct`, `enum`, `trait`, generic, `impl`, `Vec`, ownership, borrowing, `Option`, `Result`, `match`, custom error, 파일 I/O, serde JSON 변환, GlueSQL 저장소 교체, SQL 결과 매핑을 실제 코드 안에서 볼 수 있게 한다.
