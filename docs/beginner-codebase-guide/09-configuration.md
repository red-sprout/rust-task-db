...
별도 데이터베이스 설정 파일은 코드에서 확인되지 않음. Step 17 현재도 `src/main.rs`가 `GlueSqlTaskRepository::persistent("data/rust-task-db")`를 호출한다.
...
Step 17 현재 `[dependencies]`에는 `serde`, `serde_json`, `gluesql`, `futures`를 둔다. `gluesql` feature에는 `gluesql_memory_storage`와 `gluesql_sled_storage`가 포함된다. Query Execution 분석을 추가했지만 새 외부 crate는 추가하지 않았다.
