...
- 읽을 파일: [16-run-guide.md](16-run-guide.md)
...
- 다음에 읽을 파일: [src/repository/mod.rs](../../src/repository/mod.rs)

## 2단계: repository 구조 이해

- 읽을 파일: [src/repository/mod.rs](../../src/repository/mod.rs)
...
- 다음에 읽을 파일: [src/repository/gluesql_repository.rs](../../src/repository/gluesql_repository.rs)
...
## 2-1-1단계: Query Execution 흐름 이해

- 읽을 파일: [19-query-execution.md](19-query-execution.md)
- 읽는 이유: Todo 명령별 SQL 생성과 `Payload`가 `Task`, `TaskStats`, `SqlResult`로 변환되는 흐름을 한 번에 본다.
- 이 파일에서 봐야 할 코드: `execute`, `select_tasks`, `row_to_task`, `select_count`, `payload_to_sql_result`, `value_to_string`
- 이 파일을 읽고 나면 알아야 하는 것: Todo 전용 명령과 사용자 SQL 명령은 같은 `execute`를 지나지만 반환 타입 변환 경계가 다르다.
- 다음에 읽을 파일: [20-storage-comparison.md](20-storage-comparison.md)

## 2-1-2단계: Storage별 차이 이해

- 읽을 파일: [20-storage-comparison.md](20-storage-comparison.md)
- 읽는 이유: 현재 코드에 실제로 연결된 storage와 비교 문서에만 등장하는 storage를 구분한다.
- 이 파일에서 봐야 할 코드: `JsonTaskRepository::execute_sql`, `GlueSqlTaskRepository::new`, `GlueSqlTaskRepository::persistent`
- 이 파일을 읽고 나면 알아야 하는 것: `JsonTaskRepository`, `MemoryStorage`, `SledStorage`는 현재 코드에서 확인되고, `SharedMemoryStorage`, `JsonStorage`, `MongoStorage`, `CompositeStorage`는 현재 코드에 도입하지 않았다.
- 다음에 읽을 파일: [src/repl.rs](../../src/repl.rs)

## 2-2단계: REPL 입력 흐름 이해

- 읽을 파일: [src/repl.rs](../../src/repl.rs)
...
- 다음에 읽을 파일: [src/task.rs](../../src/task.rs)

## 3단계: 데이터 구조 이해

- 읽을 파일: [src/task.rs](../../src/task.rs)
...
- 다음에 읽을 파일: [src/command.rs](../../src/command.rs)

## 4단계: 명령 모델 이해

- 읽을 파일: [src/command.rs](../../src/command.rs)
...
- 다음에 읽을 파일: [src/cli.rs](../../src/cli.rs)

## 5단계: CLI parsing 이해

- 읽을 파일: [src/cli.rs](../../src/cli.rs)
...
- 다음에 읽을 파일: [src/main.rs](../../src/main.rs)의 `match command`

## 6단계: 프로그램 실행 구조 이해

- 읽을 파일: [src/main.rs](../../src/main.rs)
...
- 다음에 읽을 파일: [03-runtime-flow.md](03-runtime-flow.md)
...
- 다음에 읽을 파일: [05-file-by-file/05-test-files.md](05-file-by-file/05-test-files.md)
...
- 다음에 읽을 파일: [10-error-handling.md](10-error-handling.md)
