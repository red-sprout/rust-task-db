...
현재 Step 11에서는 `add` 처리가 CLI parsing, GlueSQL repository 생성, service 생성, Todo 추가, SQL 실행으로 나뉜다. 실패가 생기면 `AppError`로 표현된다.
...
-> GlueSqlTaskRepository::new로 GlueSQL MemoryStorage와 tasks table을 만든다.
...
Step 3부터 Step 7까지는 `tasks.json`에 저장했다. 현재 Step 11에서는 `main.rs`가 `TaskService`를 호출하고, 실제 저장 책임은 `GlueSqlTaskRepository`가 맡는다. 실패는 `AppError`로 표현한다.

```text
cargo run -- add "Rust 공부"
-> GlueSQL MemoryStorage에 INSERT

cargo run -- list
-> 새 프로세스라 MemoryStorage가 비어 있을 수 있음
```

그래서 Step 11에서는 별도 `cargo run` 명령끼리 데이터가 이어지지 않을 수 있다. 같은 repository 인스턴스 안에서 이어지는 흐름은 테스트나 `cargo run -- repl`로 확인한다.

## `load_tasks`는 전체적으로 무슨 일을 하나?

`load_tasks`는 `tasks.json` 파일을 읽어서 Rust의 `Vec<Task>`로 바꾸는 함수다. 현재 Step 11에서는 기본 실행 경로가 아니라, 보존된 `JsonTaskRepository` 안에 있다.
...
```rust
let repository = GlueSqlTaskRepository::new()?;
...
현재 실행에서는 R이 GlueSqlTaskRepository다.
```
...
그래서 REPL 안에서는 같은 GlueSQL `MemoryStorage`가 유지된다.
