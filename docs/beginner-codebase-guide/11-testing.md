...
Step 14는 새 CLI 명령을 추가하지 않고 GlueSQL storage transaction/동시성 관찰 테스트를 추가한다.

| 구분 | Step 13 | Step 14 |
| --- | --- | --- |
| 기본 CLI 저장소 | `SledStorage` | `SledStorage` |
| 테스트 개수 | 58개 | 62개 |
| 새 테스트 | 없음 | `memory_storage_rejects_explicit_transactions`, `sled_storage_rolls_back_uncommitted_insert`, `sled_storage_keeps_repeatable_read_snapshot_until_commit`, `sled_storage_reports_database_locked_for_competing_writes` |
| 검증하는 것 | 문서 정합성 | storage별 transaction 차이, rollback, snapshot, write lock 충돌 |

## Step 15에서 달라진 점

Step 15는 새 CLI 명령을 추가하지 않고 GlueSQL engine/storage adapter 분석을 문서화하며, storage 지원 경계 테스트를 보강한다.

| 구분 | Step 14 | Step 15 |
| --- | --- | --- |
| 기본 CLI 저장소 | `SledStorage` | `SledStorage` |
| 테스트 개수 | 62개 | 65개 |
| 새 테스트 | transaction 관찰 4개 | `json_repository_reports_sql_as_unsupported`, `sled_storage_commits_explicit_transaction`, `sled_storage_rejects_nested_transaction` |
| 검증하는 것 | rollback, snapshot, write lock | unsupported SQL boundary, explicit commit, nested transaction 실패 |
...
- SQL 미지원 repository가 `AppError::Unsupported`를 반환하는가
- `MemoryStorage`가 명시적 transaction을 거부하는가
- `SledStorage`가 rollback, commit, snapshot, write lock 충돌, nested transaction 실패를 현재 Todo table에서 보여주는가
- 같은 Sled DB를 동시에 관찰할 때 `SledStorage::clone()`을 사용하는가
...
현재 Step 16의 기능 테스트 수는 총 65개다. Step 16은 새 CLI 명령이나 새 테스트를 추가하지 않고 Minimal Custom Storage 책임을 문서로 분석한다. Step 15에서 추가한 GlueSQL engine/storage adapter 분석과 storage 지원 경계 테스트는 그대로 유지한다.
...
| 테스트가 검증하는 것 | GlueSQL `MemoryStorage`에서 SQL Todo 기능이 동작하고, `SledStorage`에서 데이터 유지, rollback, commit, snapshot, write lock, nested transaction 흐름이 관찰되는지 |
...
## 대표 SledStorage transaction 테스트 해설
...
```text
SledStorage를 한 번 연다.
-> clone으로 같은 DB를 가리키는 storage handle을 만든다.
-> 각 handle을 Glue::new(...)에 넣어 reader/writer repository를 만든다.
-> BEGIN/COMMIT/ROLLBACK SQL로 transaction 동작을 관찰한다.
```

주의: 같은 path를 `SledStorage::new(path)`로 동시에 두 번 열면 OS 파일 락 때문에 실패할 수 있다. 현재 테스트는 GlueSQL 예제와 같은 clone 패턴을 사용한다.

Step 15에서 추가된 transaction 경계:

- `sled_storage_commits_explicit_transaction`: `BEGIN` 후 insert하고 `COMMIT`하면 row가 남는다.
- `sled_storage_rejects_nested_transaction`: 이미 transaction이 열린 상태에서 다시 `BEGIN`하면 `AppError::GlueSql`로 실패한다.
- `json_repository_reports_sql_as_unsupported`: GlueSQL repository가 아닌 `JsonTaskRepository`는 SQL을 `AppError::Unsupported`로 거부한다.
