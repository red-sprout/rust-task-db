...
`rust-task-db`는 Rust 문법과 간단한 아키텍처 분리를 단계별로 학습하기 위한 CLI Todo 프로젝트다. 현재는 `Command` enum, CLI parser, service layer, repository trait, custom error, GlueSQL `MemoryStorage`, SQL 실행 모드, REPL 모드, 테스트 보강까지 구현되어 있다.
...
| 활성 저장소 | `GlueSqlTaskRepository` + GlueSQL `MemoryStorage` |
| 보존된 저장소 | `JsonTaskRepository` + `tasks.json` |
| 테스트 | `cargo test` 기준 57개 |
...
현재 기본 실행 경로는 GlueSQL `MemoryStorage`를 사용한다. 그래서 아래처럼 명령을 서로 다른 프로세스로 나눠 실행하면 이전 데이터가 유지되지 않을 수 있다.
...
같은 저장소 인스턴스 안에서 데이터가 이어지는 흐름은 테스트와 REPL로 확인한다.

```bash
cargo run -- repl
```

`tasks.json`은 이전 단계의 `JsonTaskRepository`를 보존하기 위해 남아 있다. 현재 기본 실행 저장소는 `tasks.json`이 아니다.
...
- `src/repository/gluesql_repository.rs`: 현재 활성 GlueSQL 저장소
